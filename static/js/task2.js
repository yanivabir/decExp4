...
function rest(order) {
  ...
  var table_order = order.table_order,
    ...
    exp_blocks_n_trials = order.n_trials_sess2_4[0];

  exp_colors = colors.slice(n_blocks_sess1 + 1, n_blocks_sess1 + 1 + n_exp_blocks);
  ...
  for (i = 0; i < n_exp_blocks; i++) {
    ...
    for (j = 1; j <= n_tables; j++) {
      st.push(table_order[n_blocks_sess1 + i] + j);
      stt.push(table_order[n_blocks_sess1 + i] + j + '.jpg');
    }
  ...
  // And then decks
  var ptrn_start_indx = 4 + n_blocks_sess1 * n_tables * 2,
  ...
  var debrief = [{
    ...
    },
    {
      ...
      on_load: function() {
        var d = new Date;
        saveData('S' + PID + '_sess2_' + d.toISOString().slice(0,10),
          jsPsych.data.get().ignore('stimulus').csv(),
          function() {
            saveData('S' + PID + '_sess2' + '_plan', trial_plan,
              function() {
                saveData('S' + PID + '_sess2' + '_int',
    ...
    },
    {
      ...
      <p>Your completion code for this study is: <br> <b>6BN2SF3</b></p>\
